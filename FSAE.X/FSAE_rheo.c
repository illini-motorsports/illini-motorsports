...
void set_rheo(uint8_t val, SPIConn *conn) { send_spi(val, conn); }

void send_all_rheo(uint16_t msg) {
  int i;
  for (i = 0; i < rheoConnIdx; i++) {
    *(rheoConnections[i].cs_lat) &=
        ~(1 << (rheoConnections[i].cs_num)); // Set CS Low
  }

  rheoConnections[0].send_fp(msg);

  for (i = 0; i < rheoConnIdx; i++) {
    *(rheoConnections[i].cs_lat) |= 1
                                    << rheoConnections[i].cs_num; // Set CS High
  }
}
