...
SPIConn *init_mcp23s17(uint8_t bus, uint32_t *cs_lat, uint8_t cs_num) {
  ...
  defaultConfig.MIRROR =
      1; // An interrupt on either bank will cause both INT pins to activate
  ...
  mcp23s17_write_reg_ctrl(MCP23S17_IPOL, 0x00,
                          currConn); // Do not reverse polarity
  mcp23s17_write_reg_ctrl(MCP23S17_GPINTEN, 0x00, currConn); // no interrupts
  mcp23s17_write_reg_ctrl(MCP23S17_DEFVAL, 0x00,
                          currConn); // default value of 0
  mcp23s17_write_reg_ctrl(MCP23S17_INTCON, 0x00,
                          currConn); // use prev value for interrupts
...
uint8_t mcp23s17_read_reg(uint8_t addr, SPIConn *conn) {
  uint32_t control_byte =
      (MCP23S17_CONTROL << 4) | (MCP23S17_HW_ADDR << 1) | MCP23S17_READ;
...
void mcp23s17_write_reg(uint8_t addr, uint8_t value, SPIConn *conn) {
  uint8_t control_byte =
      (MCP23S17_CONTROL << 4) | (MCP23S17_HW_ADDR << 1) | MCP23S17_WRITE;
