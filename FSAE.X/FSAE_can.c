 ...
 * handler function can be called up to 32 times before this function will
 * return.
 *
...
void init_can(void) {
  ...
  // C1CON
  C1CONbits.CANCAP =
      1; // CAN Message Receive Timestamp Timer Capture Enable (Enabled)
  C1CONbits.SIDL = 0; // CAN Stop in Idle (CAN continues operation when system
                      // enters idle mode)
   ...
   *
   * Note: A BRP value of 9 will cause the bus to run at a rate of 500kbps.
   * Setting BRP to 4 gives the correct rate of 1Mpbs. I'm not sure how the math
   * on this checks out, but it works.
   */
  ...
  C1CFGbits.SAM = 1;    // Sample of the CAN Bus Line (Bus line is sampled three
                        // times at the sample point)
  C1CFGbits.SJW = 2;    // Synchronization Jump Width (Length is 3 * Tq)
  C1CFGbits.WAKFIL = 0; // CAN Bus Line Filter Enable (CAN bus line filter is
  ...
  C1FIFOCON0bits.DONLY = 0; // Store Message Data Only (Full message is stored,
  ...
  /**
   * Here, we set up the first CAN acceptance filter to allow any messages with
   * standard identifiers to be sent to the receive buffer (FIFO 0). All
   * messages with extended identifiers are ignored.
   */

  // CAN1 Acceptance Filter 0
  C1RXF0bits.SID =
      0; // Standard Identifier bits (Must be all zeros (minus mask) to match)
  C1RXF0bits.EXID =
      0; // Extended Identifier Enable bits (Match only standard identifier)
  C1RXF0bits.EID =
      0; // Extended Identifier bits (Must be all zeros (minus mask) to match)

  // CAN1 Acceptance Filter Mask 0
  C1RXM0bits.SID =
      0; // Standard Identifier bits (No bits included in filter comparison)
  C1RXM0bits.MIDE = 1; // Identifier Receive Mode bit (Match only message types
                       // that correspond to EXID bit in filter)
  C1RXM0bits.EID = 0x3FFFF; // Extended Identifier bits (All bits included in
                            // filter comparison)

  // CAN1 Filter 0 - Filter must be disabled before modification
  C1FLTCON0bits.FLTEN0 = 0; // Filter 0 Enable bit (Filter is disabled)
  C1FLTCON0bits.MSEL0 =
      0b00; // Filter 0 Mask Select bits (Acceptance Mask 0 selected)
  C1FLTCON0bits.FSEL0 = 0b00000; // FIFO Selection bits (Message matching filter
                                 // is stored in FIFO 0)
  C1FLTCON0bits.FLTEN0 = 1;      // Filter 0 Enable bit (Filter is enabled)

  // Set up CAN1 Interrupt
  IFS4bits.CAN1IF =
      0; // CAN1 Interrupt Flag Status (No interrupt request has occurred)
  IPC37bits.CAN1IP = 4; // CAN1 Interrupt Priority (Interrupt priority is 4)
  IPC37bits.CAN1IS =
      3; // CAN1 Interrupt Subpriority (Interrupt subpriority is 3)
...
/**
 * Generic parsing function for a numeric CAN field.  Used in auto-generated
 * parisng functions
 */
double CAN_extract_numeric(uint8_t *data, uint8_t pos, uint8_t length,
                           Endian endianness, uint8_t sgn, double scl,
                           double off) {
...
uint32_t CAN_parse_bytes(uint8_t *data, uint8_t pos, uint8_t length,
                         Endian endianness) {
  uint32_t rawField = 0;
  if (endianness == BIG) {
    switch (length) {
