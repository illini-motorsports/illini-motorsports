...
void init_spi(uint8_t bus, double mhz, uint8_t size, uint8_t mode) {
  ...
  case 1:
    ...
    SPI1CONbits.MCLKSEL = 0; // Master Clock Enable bit (PBCLK2 is used by the
                             // Baud Rate Generator)
    SPI1CONbits.SIDL =
    ...
    SPI1CONbits.CKE =
        cke; // SPI Clock Edge Select (Serial output data changes on transition
             // from active clock state to idle clock state)
    SPI1CONbits.SMP = smp; // SPI Data Input Sample Phase (Input data sampled at
                           // middle of output time)
    SPI1CONbits.CKP =
        ckp; // Clock Polarity Select (Idle state for clock is a high level)
  ...
  case 2:
    ...
    SPI2CONbits.MCLKSEL = 0; // Master Clock Enable bit (PBCLK2 is used by the
                             // Baud Rate Generator)
    SPI2CONbits.SIDL =
        0; // Stop in Idle Mode bit (Continue operation in Idle mode)
    ...
    SPI2CONbits.CKE =
        cke; // SPI Clock Edge Select (Serial output data changes on transition
             // from active clock state to idle clock state)
    SPI2CONbits.SMP = smp; // SPI Data Input Sample Phase (Input data sampled at
                           // middle of output time)
    SPI2CONbits.CKP =
        ckp; // Clock Polarity Select (Idle state for clock is a high level)
  ...
  case 3:
    ...
    SPI3CONbits.MCLKSEL = 0; // Master Clock Enable bit (PBCLK2 is used by the
                             // Baud Rate Generator)
    SPI3CONbits.SIDL =
        0; // Stop in Idle Mode bit (Continue operation in Idle mode)
    ...
    SPI3CONbits.CKE =
        cke; // SPI Clock Edge Select (Serial output data changes on transition
             // from active clock state to idle clock state)
    SPI3CONbits.SMP = smp; // SPI Data Input Sample Phase (Input data sampled at
                           // middle of output time)
    SPI3CONbits.CKP =
        ckp; // Clock Polarity Select (Idle state for clock is a high level)
  ...
  case 5:
    ...
    SPI5CONbits.MCLKSEL = 0; // Master Clock Enable bit (PBCLK2 is used by the
                             // Baud Rate Generator)
    SPI5CONbits.SIDL =
        0; // Stop in Idle Mode bit (Continue operation in Idle mode)
    ...
    SPI5CONbits.CKE =
        cke; // SPI Clock Edge Select (Serial output data changes on transition
             // from active clock state to idle clock state)
    SPI5CONbits.SMP = smp; // SPI Data Input Sample Phase (Input data sampled at
                           // middle of output time)
    SPI5CONbits.CKP =
        ckp; // Clock Polarity Select (Idle state for clock is a high level)
  ...
  case 6:
    ...
    SPI6CONbits.MCLKSEL = 0; // Master Clock Enable bit (PBCLK2 is used by the
                             // Baud Rate Generator)
    SPI6CONbits.SIDL =
        0; // Stop in Idle Mode bit (Continue operation in Idle mode)
    ...
    SPI6CONbits.CKE =
        cke; // SPI Clock Edge Select (Serial output data changes on transition
             // from active clock state to idle clock state)
    SPI6CONbits.SMP = smp; // SPI Data Input Sample Phase (Input data sampled at
                           // middle of output time)
    SPI6CONbits.CKP =
        ckp; // Clock Polarity Select (Idle state for clock is a high level)
...
uint64_t send_spi_triple(uint64_t value, SPIConn *conn) {
  *(conn->cs_lat) &= ~(1 << (conn->cs_num));              // Set CS Low
  ...
  uint64_t buff =
      (buff1 << 32) | (buff2 << 16) | buff3; // combine the return values
  *(conn->cs_lat) |= 1 << conn->cs_num;      // Set CS High
  return buff;
}
...
void send_spi_CAN(uint64_t value[], uint64_t rec[], SPIConn *conn) {
  *(conn->cs_lat) &= ~(1 << (conn->cs_num)); // Set CS Low
  ...
  for (i = 0; i < 64; i++) {
    ...
    rec[i] = (buff1 << 48) | (buff2 << 32) | (buff3 << 16) |
             buff4; // combine the return values
  }
  *(conn->cs_lat) |= 1 << conn->cs_num; // Set CS High
}
