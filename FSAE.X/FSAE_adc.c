...
void init_adc(void (*specific_init)(void)) {
  ...
  ADCCON1bits.STRGSRC =
      1; // Scan Trigger Source Select (Global Software Trigger (GSWTRG))
  ...
  while (!ADCANCONbits.WKRDY7)
    ;                     // Wait until ADC7 analog and bias circuitry is ready
 ...
 * been triggered in the background. This should be handled by a timer
 * interrupt.
 *
