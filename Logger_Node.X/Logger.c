        ...
        if(!can_err && millis - rpm_tmr < RPM_WAIT && rpm > RPM_THRESH) {
            ...
            while(1) {
                ...
                if(millis - CAN_send_tmr > CAN_PERIOD) {
                    ...
                    ECANSendMessage(LOGGER_ID, filename_msg, 2,
...
void read_CAN_buffers(void) {
    ...
    for(i = 0; i < MSGS_READ; i++) {
        ...
        if(id == MOTEC_ID + 0x0) {
            ((unsigned char*)&rpm)[0] = data[ENG_RPM_BYTE + 1];
            ((unsigned char*)&rpm)[1] = data[ENG_RPM_BYTE];
            rpm_tmr = millis;
        }
        ...
        if(dlc > 0) {
            ...
            // Timestamp
            msg[10] = ((unsigned char*) curr_stamp_reg)[0 + (i * 2)];
            msg[11] = ((unsigned char*) curr_stamp_reg)[1 + (i * 2)];
            msg[12] = ((unsigned char*) curr_stamp_sec)[0 + (i * 2)];
            msg[13] = ((unsigned char*) curr_stamp_sec)[1 + (i * 2)];

            // Write msg to a buffer
            append_write_buffer(msg, 14);
        }
    }
}
