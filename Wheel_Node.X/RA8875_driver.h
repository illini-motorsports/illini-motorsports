...
SPIConn *
init_ra8875(uint8_t bus, uint32_t *cs_lat,
            uint8_t cs_num); // initializes spi and calls other init functions
void reset_init(); // Intended to restart ra8875 after it has entered error
                   // state
uint8_t
ra8875_send_spi(uint8_t val1, uint8_t val2,
                SPIConn *conn); // Specialized send_spi function for RA8875
...
void textSetCursor(uint16_t x,
                   uint16_t y); // Sets the coordinates of the cursor
...
void sevenSegmentMultDigit(uint16_t x, uint16_t y, uint16_t w, uint16_t numNums,
...
void writeCoordinates(uint8_t s_reg, uint16_t x,
                      uint16_t y); // Coordinate helper function
...
uint8_t
isDisplayOn(); // used to determine if display is in an unresponsive error state
