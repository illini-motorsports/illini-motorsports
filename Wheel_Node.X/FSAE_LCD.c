...
uint16_t shiftRPM[7] = {12444, 11959, 12155, 12100,
                        11150, 13000, 13000}; // theres only gears 1-5
...
void initAllScreens(void) {
  ...
  initScreenItem(&raceScreenItems[3], 20, 90, 30, redrawDigit,
                 &motecDataItems[OIL_TEMP_IDX], MIN_REFRESH);
  initScreenItem(&raceScreenItems[4], 360, 90, 30, redrawDigit,
                 &motecDataItems[ENG_TEMP_IDX], MIN_REFRESH);
  initScreenItem(&raceScreenItems[5], 20, 210, 30, redrawDigit,
                 &motecDataItems[OIL_PRES_IDX], MIN_REFRESH);
  initScreenItem(&raceScreenItems[6], 350, 210, 30, redrawDigit,
                 &pdmDataItems[VBAT_RAIL_IDX], MIN_REFRESH);
  initScreenItem(&raceScreenItems[7], 200, 20, 80, redrawGearPos,
                 &gcmDataItems[GEAR_IDX], MIN_REFRESH);
  // initScreenItem(&raceScreenItems[8], 20, 30, 15,
  // redrawShiftLightsRPM,*shiftLights);
  initScreenItem(&raceScreenItems[9], 20, 30, 15, redrawKILLCluster,
                 &pdmDataItems[KILL_SWITCH_IDX], MIN_REFRESH);
  initScreenItem(&raceScreenItems[10], 180, 210, 30, redrawDigit,
                 &motecDataItems[LAMBDA_IDX], MIN_REFRESH);
  //  initScreenItem(&raceScreenItems[10], 180, 210, 30, redrawDigit,
  //  &motecDataItems[THROTTLE_POS_IDX], MIN_REFRESH); //debug throttle instead
  //  of lambda
  ...
  initScreenItem(&pdmDrawItems[1], 10, 30, 15, redrawDigit,
                 &pdmDataItems[IGN_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[2], 85, 30, 15, redrawDigit,
                 &pdmDataItems[INJ_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[3], 160, 30, 15, redrawDigit,
                 &pdmDataItems[FUEL_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[4], 235, 30, 15, redrawDigit,
                 &pdmDataItems[ECU_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[5], 310, 30, 15, redrawDigit,
                 &pdmDataItems[WTR_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[6], 385, 30, 15, redrawDigit,
                 &pdmDataItems[FAN_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[7], 10, 80, 15, redrawDigit,
                 &pdmDataItems[AUX_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[8], 85, 80, 15, redrawDigit,
                 &pdmDataItems[PDLU_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[9], 160, 80, 15, redrawDigit,
                 &pdmDataItems[PDLD_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[10], 235, 80, 15, redrawDigit,
                 &pdmDataItems[ABS_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[11], 310, 80, 15, redrawDigit,
                 &pdmDataItems[BVBAT_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[15], 385, 80, 15, redrawDigit,
                 &pdmDataItems[STR_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[16], 10, 130, 15, redrawDigit,
                 &pdmDataItems[IGN_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[17], 85, 130, 15, redrawDigit,
                 &pdmDataItems[INJ_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[18], 160, 130, 15, redrawDigit,
                 &pdmDataItems[FUEL_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[19], 235, 130, 15, redrawDigit,
                 &pdmDataItems[ECU_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[20], 310, 130, 15, redrawDigit,
                 &pdmDataItems[WTR_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[21], 385, 130, 15, redrawDigit,
                 &pdmDataItems[FAN_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[22], 10, 180, 15, redrawDigit,
                 &pdmDataItems[AUX_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[23], 85, 180, 15, redrawDigit,
                 &pdmDataItems[PDLU_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[24], 160, 180, 15, redrawDigit,
                 &pdmDataItems[PDLD_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[25], 235, 180, 15, redrawDigit,
                 &pdmDataItems[ABS_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[26], 310, 180, 15, redrawDigit,
                 &pdmDataItems[BVBAT_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[27], 160, 230, 15, redrawDigit,
                 &pdmDataItems[FUEL_CUT_P_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[28], 235, 230, 15, redrawDigit,
                 &pdmDataItems[FAN_CUT_P_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[29], 310, 230, 15, redrawDigit,
                 &pdmDataItems[WTR_CUT_P_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[30], 385, 230, 15, redrawDigit,
                 &pdmDataItems[ECU_CUT_P_IDX], MIN_REFRESH);
  initScreenItem(&pdmDrawItems[31], 20, 30, 15, redrawShiftLightsRPM,
                 (volatile dataItem *)shiftLights, MIN_REFRESH);
  initScreenItem(&pdmDrawItems[32], 20, 30, 15, redrawKILLCluster,
                 &pdmDataItems[KILL_SWITCH_IDX], MIN_REFRESH);

  allScreens[PDM_GRID_SCREEN] = &pdmGridScreen;
  pdmGridScreen.items = pdmGridItems;
  pdmGridScreen.len = 13;
  initScreenItem(&pdmGridItems[1], 10, 30, 15, redrawDigit,
                 &pdmDataItems[IGN_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[2], 85, 30, 15, redrawDigit,
                 &pdmDataItems[INJ_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[3], 160, 30, 15, redrawDigit,
                 &pdmDataItems[FUEL_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[4], 235, 30, 15, redrawDigit,
                 &pdmDataItems[ECU_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[5], 310, 30, 15, redrawDigit,
                 &pdmDataItems[WTR_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[6], 385, 30, 15, redrawDigit,
                 &pdmDataItems[FAN_DRAW_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[7], 10, 70, 15, redrawDigit,
                 &pdmDataItems[IGN_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[8], 85, 70, 15, redrawDigit,
                 &pdmDataItems[INJ_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[9], 160, 70, 15, redrawDigit,
                 &pdmDataItems[FUEL_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[10], 235, 70, 15, redrawDigit,
                 &pdmDataItems[ECU_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[11], 310, 70, 15, redrawDigit,
                 &pdmDataItems[WTR_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmGridItems[12], 385, 70, 15, redrawDigit,
                 &pdmDataItems[FAN_CUT_IDX], MIN_REFRESH);

  allScreens[PDM_CUT_SCREEN] = &pdmCutScreen;
  pdmCutScreen.items = pdmCutItems;
  pdmCutScreen.len = 21;
  initScreenItem(&pdmCutItems[0], 10, 50, 15, redrawDigit,
                 &pdmDataItems[PCB_TEMP_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[1], 85, 50, 15, redrawDigit,
                 &pdmDataItems[IC_TEMP_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[2], 235, 200, 15, redrawDigit,
                 &pdmDataItems[VBAT_RAIL_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[3], 330, 200, 15, redrawDigit,
                 &pdmDataItems[V12_RAIL_IDX], MIN_REFRESH);
  ...
  initScreenItem(&pdmCutItems[7], 85, 100, 15, redrawDigit,
                 &pdmDataItems[IGN_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[8], 160, 100, 15, redrawDigit,
                 &pdmDataItems[INJ_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[9], 235, 100, 15, redrawDigit,
                 &pdmDataItems[FUEL_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[10], 160, 50, 15, redrawDigit,
                 &pdmDataItems[ECU_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[12], 10, 150, 15, redrawDigit,
                 &pdmDataItems[FAN_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[13], 85, 150, 15, redrawDigit,
                 &pdmDataItems[AUX_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[14], 160, 150, 15, redrawDigit,
                 &pdmDataItems[PDLU_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[15], 235, 150, 15, redrawDigit,
                 &pdmDataItems[PDLD_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[16], 310, 150, 15, redrawDigit,
                 &pdmDataItems[ABS_CUT_IDX], MIN_REFRESH);
  initScreenItem(&pdmCutItems[17], 385, 150, 15, redrawDigit,
                 &pdmDataItems[BVBAT_CUT_IDX], MIN_REFRESH);
  ...
  initScreenItem(&throttleItems[0], 50, 40, 50, redrawDigit,
                 &motecDataItems[THROTTLE_POS_IDX], MIN_REFRESH);
  initScreenItem(&throttleItems[1], 350, 70, 100, redrawGearPos,
                 &gcmDataItems[GEAR_IDX], MIN_REFRESH);
  initScreenItem(&throttleItems[2], 50, 150, 50, redrawDigit,
                 &motecDataItems[ENG_RPM_IDX], MIN_REFRESH);
  ...
  imuScreen.len = 3;
  initScreenItem(&imuItems[0], 360, 107, 20, redrawDigit,
                 &imuDataItems[LATERAL_G_IDX], MIN_REFRESH);
  initScreenItem(&imuItems[1], 203, 235, 20, redrawDigit,
                 &imuDataItems[LONGITUDINAL_G_IDX], MIN_REFRESH);
  initScreenItem(&imuItems[2], 239, 97, 90, redrawGforceGraph,
                 (volatile dataItem *)gForce, 10);
...
void initScreenItem(screenItem *item, uint16_t x, uint16_t y, uint16_t size,
                    double (*redrawItem)(screenItemInfo *, volatile dataItem *,
                                         double),
                    volatile dataItem *data, uint32_t interval) {
  item->info.x = x;
  item->info.y = y;
  item->info.size = size;
  item->currentValue = 1;
...
void refreshScreenItems(void) {
  ...
  for (i = 0; i < currScreen->len; i++) {
    ...
    if (data && millis - currItem->refreshTime >= currItem->refreshInterval) {
      uint8_t warning = data->thresholdDir ? data->value >= data->warnThreshold
                                           : data->value <= data->warnThreshold;
      ...
      currItem->currentValue = currItem->redrawItem(
          &currItem->info, currItem->data, currItem->currentValue);
...
// Redraw General Data
double redrawDigit(screenItemInfo *item, volatile dataItem *data,
                   double currentValue) {
  uint8_t warning = data->thresholdDir ? data->value >= data->warnThreshold
                                       : data->value <= data->warnThreshold;
  uint8_t error = data->thresholdDir ? data->value >= data->errThreshold
                                     : data->value <= data->errThreshold;
  ...
  uint16_t fillWidth = (item->size) * (wholeNums + decNums) +
                       (item->size) * 0.2 * (wholeNums + decNums - 1);
  if (decNums) {
    fillWidth += (item->size) / 5;
  }
  fillRect(item->x, item->y, fillWidth, (item->size) * 1.75, fillColor);
  if (!stale && (!error || millis % 500 >
                               200)) { // draw number if normal, or blink at 1hz
    sevenSegmentDecimal(item->x, item->y, item->size, wholeNums + decNums,
                        decNums, numColor, data->value);
  }
  return data->value;
}

// For Single Digits with Error and Neutral Displays
double redrawGearPos(screenItemInfo *item, volatile dataItem *data,
                     double currentValue) {
  ...
  if (data->value == 0) {
    ...
    sevenSegmentDigit(item->x, item->y, item->size, foregroundColor,
                      data->value);
  }
  return data->value;
}
...
double redrawFanSw(screenItemInfo *item, volatile dataItem *data,
                   double currentValue) {
...
double redrawGCMMode(screenItemInfo *item, volatile dataItem *data,
                     double currentValue) {
...
double redrawWTRPumpSw(screenItemInfo *item, volatile dataItem *data,
                       double currentValue) {
  volatile dataItem **dataArray = (volatile dataItem **)data;
  // Override
  if (dataArray[0]->value) {
...
double redrawFUELPumpSw(screenItemInfo *item, volatile dataItem *data,
                        double currentValue) {
  volatile dataItem **dataArray = (volatile dataItem **)data;
  // Override
  if (dataArray[0]->value) {
...
double redrawTireTemp(screenItemInfo *item, volatile dataItem *data,
                      double currentValue) {
  volatile dataItem **dataArray = (volatile dataItem **)data;
  ...
  fillCircleSquare(x, y, width * 2, height, width,
                   tempColor(dataArray[0]->value));
  fillCircleSquare(x + (2 * width), y, width * 2, height, width,
                   tempColor(dataArray[3]->value));
...
double redrawSPBar(screenItemInfo *item, volatile dataItem *data,
                   double currentValue) {
  if (data->value == currentValue) {
    return data->value;
  }
  ...
  if (data->value > MIN_SUS_POS) {
    uint16_t height = ((item->size * 5) / (MAX_SUS_POS - MIN_SUS_POS)) *
                      (data->value - MIN_SUS_POS);
    if (height > MAX_SUS_POS) {
      height = MAX_SUS_POS;
    }
    fillRect(item->x, item->y - (item->size * 5) + height, item->size, height,
             RA8875_RED);
  }
  return data->value;
}
...
double redrawBrakeBar(screenItemInfo *item, volatile dataItem *data,
                      double currentValue) {
  if (data->value == currentValue) {
    return data->value;
  }
  ...
  if (data->value > MIN_BRAKE_PRESS) {
    uint16_t height =
        ((item->size * 10) / (MAX_BRAKE_PRESS - MIN_BRAKE_PRESS)) *
        (data->value - MIN_BRAKE_PRESS);
    if (height > MAX_BRAKE_PRESS) {
      height = MAX_BRAKE_PRESS;
    }
    fillRect(item->x, item->y - (item->size * 10) + height, item->size, height,
             RA8875_RED);
  }
  return data->value;
}

double redrawRotary(screenItemInfo *item, volatile dataItem *data,
                    double currentValue) {
  fillCircle(item->x, item->y, item->size, RA8875_RED);
  if (data->value == currentValue) {
    return data->value;
  }
  sevenSegmentDigit(item->x - (item->size / 2.0), item->y - (item->size / 2.0),
                    item->size, RA8875_BLACK, data->value);
  return data->value;
}
...
double redrawShiftLightsRPM(screenItemInfo *item, volatile dataItem *data,
                            double currentValue) {
  volatile dataItem **dataArray = (volatile dataItem **)data;
...
double redrawKILLCluster(screenItemInfo *item, volatile dataItem *data,
                         double currentValue) {
...
double redrawGforceGraph(screenItemInfo *item, volatile dataItem *data,
                         double currentValue) {
  int i = 0;
  volatile dataItem **dataArray = (volatile dataItem **)data;
  ...
  // determine where to erase the previous dot by mapping currentValue to
  // coordinates
  ...
  fillRect(item->x - item->size, item->y - item->size / 160, 2 * item->size,
           item->size / 80, foregroundColor);
  fillRect(item->x - item->size / 160, item->y - item->size, item->size / 80,
           2 * item->size, foregroundColor);
  ...
  // setting the "currentValue" to allow the function to erase a localized area
  // in the future
...
uint16_t tempColor(uint8_t temp) { return RA8875_RED; }
