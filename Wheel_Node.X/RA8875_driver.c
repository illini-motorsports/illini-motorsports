 ...
 ***********************Formula Functions***************************************
 */
void drawChevron(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t fg,
                 uint16_t bg) {
  int mid = (w / 2) + x;
  ...
  int i;
  for (i = 0; i < 11; i++) {
    fillTriangle(x, y + h - (w / 2) - (i * spacing), x + w,
                 y + h - (w / 2) - (i * spacing), x + (w / 2),
                 y + h - (i * spacing), fg);
    fillTriangle(x, y + h - (w / 2) - (i * spacing) - bgOffset, x + w,
                 y + h - (w / 2) - (i * spacing) - bgOffset, x + (w / 2),
                 y + h - (i * spacing) - bgOffset, bg);
  }
...
void sevenSegmentDecimal(uint16_t x, uint16_t y, uint16_t numWidth,
                         uint16_t numNums, uint16_t decDigits, uint16_t color,
                         double number) {
  if (decDigits == 0) {
  ...
  fillCircle(x + (numWidth * 1.2 * (numNums - decDigits)), y + (numWidth * 1.7),
             numWidth / 10, color);
  sevenSegmentMultDigit(x + (numWidth * 1.2 * (numNums - decDigits)) +
                            numWidth / 5,
                        y, numWidth, decDigits, color, decNum);
}

// Displays multiple digits using the seven segment helper functions
void sevenSegmentMultDigit(uint16_t x, uint16_t y, uint16_t numWidth,
                           uint16_t numNums, uint16_t color, uint16_t number) {
  uint16_t offset = numWidth * 1.2;
  int i = 0;
  for (; i < numNums; i++) {
    sevenSegmentDigit(x + (offset * (numNums - i - 1)), y, numWidth, color,
                      number % 10);
    number /= 10;
  }
}
...
void sevenSegmentDigit(uint16_t x, uint16_t y, uint16_t w, uint16_t color,
                       uint8_t digit) {
  switch (digit) {
  case 0:
...
void sevenSegment(uint16_t x, uint16_t y, uint16_t w, uint16_t color,
                  uint8_t bMask) {
  uint16_t h = w * 1.75;
  uint8_t boxD = w / 5;
  uint8_t hBoxD = w / 10;
  ...
  if (bMask & 0x01) { // Middle Horizontal Bar
    ...
    fillTriangle(x + w - boxD, y + fhOffset, x + w - hBoxD,
                 y + hBoxD + fhOffset, x + w - boxD, y + boxD + fhOffset,
                 color);
  }
  if (bMask & 0x08) { // Bottom Horizontal Bar
    ...
    fillTriangle(x + w - boxD, y + shOffset, x + w - hBoxD,
                 y + hBoxD + shOffset, x + w - boxD, y + boxD + shOffset,
                 color);
  }
  ...
  if (bMask & 0x20) { // Bottom Left Vertical Bar
    ...
    fillTriangle(x + w - boxD, y + boxD, x + w, y + boxD, x + w - hBoxD,
                 y + hBoxD, color);
    fillTriangle(x + w - boxD, y + boxD + barHeight, x + w,
                 y + boxD + barHeight, x + w - hBoxD,
  ...
  if (bMask & 0x04) { // Top Right Vertical Bar
    ...
    fillTriangle(x, y + boxD + barHeight + fhOffset, x + boxD,
                 y + boxD + barHeight + fhOffset, x + hBoxD,
...
void fillCircleSquare(int16_t x, int16_t y, int16_t w, int16_t h,
                      int16_t corner, uint16_t color) {
...
void circleSquareHelper(int16_t x, int16_t y, int16_t w, int16_t h,
                        int16_t corner, uint16_t color, uint8_t filled) {
  // Begin Coordinates
  ...
  // Draw
  if (filled) {
    writeReg(RA8875_CIRC_SQUARE_DCR,
             RA8875_CIRC_SQUARE_STRT | RA8875_CIRC_SQUARE_FILL);
  } else {
...
void ellipseHelper(int16_t xCenter, int16_t yCenter, int16_t longAxis,
                   int16_t shortAxis, uint16_t color, uint8_t filled) {
...
void textWrite(const char *buffer) { textWriteHelper(buffer, 0); }
...
uint8_t ra8875_send_spi(uint8_t val1, uint8_t val2, SPIConn *conn) {
  *(conn->cs_lat) &= ~(1 << (conn->cs_num)); // Set CS Low
  conn->send_fp(val1);
  uint8_t buff =
      conn->send_fp(val2); // only care about the response from last 8 bits
