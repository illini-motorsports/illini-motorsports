...
int32_t linearizeThermistor(double inTherm) {
  ...
  if (inTherm >= 5896) {
    return (outTherm *
            (ANALOG_CAN_SCL / 10)); // Water is way too cold, send fault value
  }
  for (i = 0; i < 20; i++) {
    if ((resRange[i] < inTherm) && (i != 19)) {
      outTherm = tempRange[i - 1] + ((tempRange[i] - tempRange[i - 1]) /
                                     (resRange[i] - resRange[i - 1])) *
                                        (inTherm - resRange[i - 1]);
      break;
    }
  }
...
void update_strain_channels(void) {
  ...
  ad7490_read_channels(strain_positive,
                       strain_connections[0]); // Read positive values
  ad7490_read_channels(strain_negative,
                       strain_connections[1]); // Read negative values
  int i;
  for (i = 0; i < 2; i++) {
...
void calc_wheel_speeds(void) {
  ...
  for (j = 0; j < 4; j++)
    if (!prev_dig_val[j] && thing[j]) {
      wheel_speeds[j] = 10000 * TIRE_DIAMETER_PER_TR /
                        (counter * SPI_DIFF_TIME); // wheel speed in fps
      counter = 0;
    }
}
...
void init_thermocouples(void) {
  ...
  thermocouple_connections[0] =
      init_max31855(3, THERMOCOUPLE_CS_LATBITS1, THERMOCOUPLE_CS_LATNUM1);
  thermocouple_connections[1] =
      init_max31855(3, THERMOCOUPLE_CS_LATBITS2, THERMOCOUPLE_CS_LATNUM2);
  thermocouple_connections[2] =
      init_max31855(3, THERMOCOUPLE_CS_LATBITS3, THERMOCOUPLE_CS_LATNUM3);
  thermocouple_connections[3] =
      init_max31855(3, THERMOCOUPLE_CS_LATBITS4, THERMOCOUPLE_CS_LATNUM4);
}
