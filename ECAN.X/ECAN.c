...
void ECANInitialize(void)
{
    ...
    #else
        _SetXtdRXFnValue(RXF2, ECAN_RXF2_VAL);
    #endif
#endif
    ...
    // Filters 6-15 are available in Modes 1 and 2 only.
            ...
            _SetStdRXFnValue(RXF8, ECAN_RXF8_VAL);
        #else
            _SetXtdRXFnValue(RXF8, ECAN_RXF8_VAL);
        #endif
    #endif
                       ...
                       ECAN_RX_MSG_FLAGS *msgFlags)
{
...

    {
      ...
      (ECAN_LIB_MODE_VAL == ECAN_LIB_MODE_FIXED) && (ECAN_FUNC_MODE_VAL == ECAN_MODE_2) )
    {
        if ( COMSTAT_NOT_FIFOEMPTY == 1 )      //if FIFO is NOT empty --> meaning there is a message to be read waiting in Bn or RXBn Buffer
        {
