 ...
 */
void __attribute__((vector(_TIMER_2_VECTOR), interrupt(IPL6SRS)))
timer2_inthnd(void) {
...
void __attribute__((vector(_TIMER_4_VECTOR), interrupt(IPL7SRS)))
timer4_inthnd(void) {
...
void __attribute__((vector(_TIMER_6_VECTOR), interrupt(IPL7SRS)))
timer6_inthnd(void) {
  periods++; // Increment periods count
